...
def threshold_found_mean(exact_match, found_pairs):
    ...
    for pair in found_pairs:
        if pair[2] >= threshold_val:
            output.append(pair)

    return output


def threshold_found_median(exact_match, found_pairs):
...
parser.add_argument("-m", "--mode", help="mean/median")
...
for file in files:
    ...
    if mode == "mean":
        output = threshold_found_mean(exact_match, found_pairs)
    elif mode == "median":
        output = threshold_found_median(exact_match, found_pairs)
    else:
