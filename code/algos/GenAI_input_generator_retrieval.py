...
parser.add_argument("-ld", "--llmdogtag", help="Dogtag description folder path for the LLM to use")
parser.add_argument("-o", "--output", help="Output folder")
parser.add_argument("-m", "--method", type=int, default=1, help="Options: [1,2], 1 - Not strict, 2 - Strict")
parser.add_argument("-s1", '--str1', action='store_true', help='dogtag is str or dict-like-structure')
parser.add_argument("-s2", '--str2', action='store_true', help='llmdogtag is str or dict-like-structure')
...
for graph_pair in graph_pairs:
    ...
    with open(os.path.join(args.llmdogtag, g1 + ".json"), "r") as f:
        g1_llmdogtags = json.load(f)

    with open(os.path.join(args.llmdogtag, g2 + ".json"), "r") as f:
        g2_llmdogtags = json.load(f)

    if args.str1 is False:
        g1_dogtags = rows_to_str(g1_dogtags)
        g2_dogtags = rows_to_str(g2_dogtags)

    if args.str2 is False:
        g1_llmdogtags = rows_to_str(g1_llmdogtags)
        g2_llmdogtags = rows_to_str(g2_llmdogtags)
    ...
    for startnode, options in tqdm(pairs.items()):
        try:
            options_filtered = list()
            startnode_dogtag = g1_llmdogtags[startnode]
            options_dogtags = list()
            for element in options:
                try:
                    options_dogtags.append(g2_llmdogtags[element[0]])
                    options_filtered.append(element)
                except Exception as e:
                    pass
            if len(options_dogtags) != 0:
                ready_prompt = get_prompt(startnode_dogtag, options_dogtags, args.method)
                input_rows.append(ready_prompt)
                top10pairs[startnode] = options_filtered
        except Exception as e:
            pass

    with open(os.path.join(args.output, f"{g1}-{g2}.json"), "w") as f:
        json.dump(input_rows, f, indent=4)

    if not os.path.exists(os.path.join(args.output, "top10pairs")):
        os.makedirs(os.path.join(args.output, "top10pairs"))
    with open(os.path.join(args.output, "top10pairs", f"{g1}-{g2}.json"), "w") as f:
        json.dump(top10pairs, f, indent=4)
